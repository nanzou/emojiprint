...
for CHROME_SUBDIR in $CHROME_PROVIDERS; do
  c:/cygwin/bin/find $CHROME_SUBDIR \( -path '*CVS*' -o -path '*.svn*' \) -prune -o -type f -print | grep -v \~ >> files
done
...
for DIR in $ROOT_DIRS; do
  mkdir $TMP_DIR/$DIR
  FILES="`c:/cygwin/bin/find $DIR \( -path '*CVS*' -o -path '*.svn*' \) -prune -o -type f -print | grep -v \~`"
  echo $FILES >> files
